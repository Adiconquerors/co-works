    ...
    public function installProject(Request $request)
    {
        ...
        if ( $request->isMethod('post') ) {
          ...
          if($request->has('sample_data'))
          {
            ...
            {
              \Artisan::call('cowork:finishsetup'); // Let us copy sample images withdata to 'uploads' folder.
          ...
          if($request->has('sample_data'))
          {
            if($request->sample_data=='no-data')
            {
              \Artisan::call('cowork:finishsetupnodata'); // Let us copy sample images withdata to 'uploads' folder.
            }
          }
    ...
    public function registerUser(Request $request)
    {
        ...
        if ( $request->isMethod('post') ) {
         ...
         $this->validate($request,$columns);
          ...
          $contact           = \App\User::where('email', $request->owner_email)->first();
          if ( $contact ) {
  ...
  function createTables(Request $request)
  {
    try{
      ...
      if( $request->has('sample_data') && $request->sample_data=='data' ) {
        ...
        $query = str_replace('http://localhost/co-workingspace/', PREFIX, $query);
