    <script type="text/javascript" src="{{PUBLIC_ASSETS}}js/markerclusterer.js"></script>
...
var isDevice = true;
    ...
    @if ( ! empty( $rec->property_address_latitude ) && ! empty( $rec->property_address_longitude ) )
    {
  ...
  <?php
    ...
    ?>
        ...
        title : '{{$rec->company}}',
        single_url : '{{route("properties.edit", [ "slug" => $rec->slug, "sub_space_type_id" => $rec->slug ])}}',
        property_id: '{{$rec->id}}',
        image : '{{$rec->cover_image}}',

        price : '{{$price_per_month}}{{ trans("custom.listings.per-month") }}',
        address : '{{ $rec->property_address }}',
        near_by_landmark : '{{ $rec->near_by_landmark }}',


        position : {
            lat : '{{$rec->property_address_latitude}}',
            lng : '{{$rec->property_address_longitude}}',
        },
        landmark_position : {
            lat : '{{$rec->near_by_landmark_latitude}}',
            lng : '{{$rec->near_by_landmark_longitude}}',
        },
    ...
    addMarkers = function(props, map, requestfrom) {
        ...
        $.each(props, function(i,prop) {
            ...
            @if ( ! empty( $single_page ) && 'yes' === $single_page )
...
@if( ! empty( $requestfrom ) && $requestfrom == 'filter' )
	managemap();
@endif
...
{
    window.location = '{{PREFIX}}properties?location=' + formatted_address + '&wstype=';
}

</script>

@include( 'home-pages.common.map-refresh' )