...
if ( 'LanguagesController' === $controller ) {
    ...
    ?>
     ...
     @if( 'Yes' === $row->is_default )
        <a href="javascript:void(0);" class="btn btn-xs btn-info" title="@lang('custom.deault-language')">@lang('global.app_default')</a>
        @else
        <a href="{{ url('admin/make-default-language/' . $row->id) }}" class="btn btn-xs btn-success" title="@lang('custom.deault-language')">@lang('global.app_make_default')</a>
        @endif   
    <?php
}
...
if ( 'CurrenciesController' === $controller ) {
    ?>
        ...
        @else
            <a href="{{ route('currency.makedefault', $row->id) }}" class="btn btn-xs btn-success">@lang('global.app_make_default')</a>
        @endif
