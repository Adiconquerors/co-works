  ...
  $(document).ready(function() {
    ...
    $(document).on('click', '#paymentSend', function(e) {
      ...
      if ('invoice-payment' == action) {
        ...
        var errors = 0;
        if (transaction_id == '') {
          $('#transaction_id').after('<p class="error">{{trans("global.invoices.messages.transaction_id")}}</p>');
          $('#transaction_id').focus();
          errors++;
        }
        
        if (total_amount == '') {
          $('#total_amount').after('<p class="error">{{trans("global.invoices.messages.total_amount")}}</p>');
          $('#total_amount').focus();
          errors++;
        }
        ...
        //$('#paymentSend').html('{{trans("global.common.save")}}');
