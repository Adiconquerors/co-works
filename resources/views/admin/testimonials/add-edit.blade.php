...
<div class="row">
  ...
  @endif
   ...
   <?php $button_name = trans('global.app_create'); ?>
      ...
      {!! Form::model($record, ['method' => 'PUT', 'route' => ['testimonials.update',$record->slug], 'files' => true,'name'=>'formTestimonialType','class'=>'formvalidation', 'enctype' => 'multipart/form-data' ]) !!}
            
      @else


      {!! Form::open(['method' => 'POST', 'route' => ['testimonials.store'], 'files' => true,'name'=>'formTestimonialType','class'=>'formvalidation', 'enctype' => 'multipart/form-data']) !!}

        @endif

        @include('admin.testimonials.form-elements', array('button_name'=> $button_name,'record'=>$record ))
