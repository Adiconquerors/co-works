       ...
       $(document).on('click', '#shortlist_properties_share', function( e ) {
         e.preventDefault();
          ...
          if ( toemail == '' ) {
           ...
           if(IsEmail(toemail)==false){
              $('#toemail').after('<p class="error"><?php echo e(trans("custom.eforms.inv-email")); ?></p>');
              $('#toemail').focus();
              errors++;
            }
            ...
            if( mail_mobile < 0 ) {
            $('#mail_mobile').after('<p class="error"> <?php echo e(trans("custom.eforms.positive-nums")); ?></p>');
            errors++;
            }

            if ( no_of_seats != '' ) {
              if(no_of_seats < 0){
                $('#no_of_seats').after('<p class="error"><?php echo e(trans("custom.eforms.positive-nums")); ?></p>');
                $('#no_of_seats').focus();
                errors++;
              }
            }
        ...
        if ( errors == 0 ) {
         ...
         $('#shortlist_loading').html("<img src='"+image+"' />").show();      
            $.ajax({
                url: '<?php echo e(route("properties.totalshortlists")); ?>',
                type:'POST',
