       ...
       $(document).on('click', '#visit_properties_share', function( e ) {
         e.preventDefault();
          ...
          var action            = $("#visit_action").val();
            ...
            ?>
              ...
              if( toname == '' ) {
              $('#visit_toname').after('<p class="error"><?php echo e(trans("custom.eforms.enter-name")); ?></p>');
              errors++;
              }

          if ( toemail == '' ) {
            $('#visit_toemail').after('<p class="error"><?php echo e(trans("global.enquires.messages.toemail")); ?></p>');
            $('#visit_toemail').focus();
            errors++;
            }
           if(IsEmail(toemail)==false){
              $('#visit_toemail').after('<p class="error"><?php echo e(trans("custom.eforms.inv-email")); ?></p>');
              $('#visit_toemail').focus();
              errors++;
            }
            ...
            if( mail_mobile == '' ) {
            $('#visit_mail_mobile').after('<p class="error"><?php echo e(trans("custom.eforms.enter-mobile")); ?></p>');
            errors++;
            }

            if( mail_mobile < 0 ) {
            $('#visit_mail_mobile').after('<p class="error"><?php echo e(trans("custom.eforms.positive-nums")); ?></p>');
            errors++;
            }
             ...
             if ( no_of_seats != '' ) {
              if(no_of_seats < 0){
                $('#visit_no_of_seats').after('<p class="error"><?php echo e(trans("custom.eforms.positive-nums")); ?></p>');
        ...
        if ( errors == 0 ) {
         ...
         $('#visit_loading').html("<img src='"+image+"' />").show();      
            $.ajax({
                url: '<?php echo e(route("properties.sendvisits")); ?>',
                type:'POST',
