  ...
  $(document).ready(function() {
    ...
    $(document).on('click', '#enquirySend', function(e) {
      ...
      if ('seeker-details' == action) {
        ...
        var errors = 0;
        if (seeker_name == '') {
          $('#seeker_name').after('<p class="error"><?php echo e(trans("global.enquires.messages.seeker_name")); ?></p>');
          $('#seeker_name').focus();
          errors++;
        }
        if (seeker_email == '') {
          $('#seeker_email').after('<p class="error"><?php echo e(trans("global.enquires.messages.seeker_email")); ?></p>');
          $('#seeker_email').focus();
          errors++;
        }
        if (seeker_phone_number == '') {
          $('#seeker_phone_number').after('<p class="error"><?php echo e(trans("global.enquires.messages.seeker_phone_number")); ?></p>');
        ...
        var errors = 0;
        if (capacity_id == '') {
          $('#capacity_id').after('<p class="error"><?php echo e(trans("global.enquires.messages.capacity")); ?></p>');
          $('#capacity_id').focus();
          errors++;
        }
        if (req_booking_months !='' && req_booking_months <= 0 ) {
          $('#req_booking_months').after('<p class="error"><?php echo e(trans("custom.eforms.val-greater")); ?></p>');
        ...
        if (booking_start_date == '') {
          $('#booking_start_date').after('<p class="error"><?php echo e(trans("global.enquires.messages.booking-start-date")); ?></p>');
        ...
        var errors = 0;
        if (amount_paid == '') {
          $('#amount_paid').after('<p class="error"><?php echo e(trans("global.enquires.messages.amount_paid")); ?></p>');
          $('#amount_paid').focus();
          errors++;
        }
        ...
        if (company_name == '') {
          $('#company_name').after('<p class="error"> <?php echo e(trans("custom.eforms.cmp-name")); ?></p>');
          errors++;
        }

        if (company_address == '') {
          $('#company_address').after('<p class="error"> <?php echo e(trans("custom.eforms.cmp-address")); ?></p>');
          errors++;
        }
        ...
        if (company_address == '') {
          $('#company_address').after('<p class="error"><?php echo e(trans("custom.eforms.cmp-name")); ?></p>');
          errors++;
        }
        ...
        var errors = 0;
        if (visit_details == '') {
          $('#visit_details').after('<p class="error"><?php echo e(trans("global.enquires.messages.visit_details")); ?></p>');
          $('#visit_details').focus();
          errors++;
        }
        ...
        var errors = 0;
        if (assigned_to == '') {
          $('#assigned_to').after('<p class="error"><?php echo e(trans("global.enquires.messages.assigned_to")); ?></p>');
          $('#assigned_to').focus();
          errors++;
        }
        ...
        if ( toemail == '' ) {
           ...
           if(IsEmail(toemail)==false){
              $('#toemail').after('<p class="invalid-email"><?php echo e(trans("global.enquires.messages.invalid-email")); ?></p>');
              $('#toemail').focus();
              errors++;
            }
        if (toname == '') {
        ...
        if (booking_amount == '') {
          $('#booking_amount').after('<p class="error"><?php echo e(trans("global.enquires.messages.booking-amount")); ?></p>');
          $('#booking_amount').focus();
          errors++;
        }
        if (mail_description == '') {
          $('#mail_description').after('<p class="error"><?php echo e(trans("global.enquires.messages.mail_description")); ?></p>');
        ...
        if ( no_of_seats == '' ) {
            $('#no_of_seats').after('<p class="error"><?php echo e(trans("global.enquires.messages.no-of-seats")); ?></p>');
            $('#no_of_seats').focus();
            errors++;
         }

         if( no_of_seats < 0 ){
            $('#no_of_seats').after('<p class="error"><?php echo e(trans("global.enquires.messages.enter-positive-values")); ?></p>');
            $('#no_of_seats').focus();
             errors++;
         }

         if ( booking_months == '' ) {
            $('#booking_months').after('<p class="error"> <?php echo e(trans("custom.eforms.req-bmonths")); ?></p>');
            $('#booking_months').focus();
            errors++;
         }

        if( booking_months < 0 ){
            $('#booking_months').after('<p class="error"><?php echo e(trans("global.enquires.messages.enter-positive-values")); ?></p>');
            $('#booking_months').focus();
             errors++;
         }  
        ...
        var errors = 0;
        if (deal_lost == '') {
          $('#deal_lost').after('<p class="error"><?php echo e(trans("global.enquires.messages.deal_lost")); ?></p>');
          $('#deal_lost').focus();
          errors++;
        }
      ...
      if (errors == 0) {
        ...
        $.ajax({
          url: '<?php echo e(route("seekerdetails.save")); ?>',
          dataType: "json",
          method: 'post',
          data: {
            action: action,
    ...
    function loadEmailTemplate(enquiry_id, action) {
      ...
      $.ajax({
        url: "<?php echo e(route('enquires.seekerdetails')); ?>",
        type: 'POST',
        data: {
          '_token': _token,
