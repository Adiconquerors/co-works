<?php
 ...
 ?>
 <script src="//maps.googleapis.com/maps/api/js?key=<?php echo e($google_api_key); ?>&libraries=geometry,places">
        </script>
  ...
  function initialize_top( id ) {
    ...
    google.maps.event.addListener(autocomplete_my, 'place_changed', function () {
            ...
            if (place.address_components) {
                for( var i = 0; i < place.address_components.length; i++ ) {
                    ...
                    if (addressType == 'administrative_area_level_1' ) { // State
                     ...
                     if (addressType == 'administrative_area_level_3' ) { // State
                      property_address_city = place.address_components[i].long_name;
                    }
        ...
        if ( id == 'search_property') {
            searchProperty( property_address_city );
        }
    });
}
function searchProperty( property_address_city )
{
    window.location = '<?php echo e(PREFIX); ?>properties?location=' + property_address_city + '&wstype=all';
}
